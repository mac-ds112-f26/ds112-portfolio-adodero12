...
#elevation, ascent, length, time
b.  What about a visualization of the *quantitative* `elevation` variable?
#bar graph because it is only measuring height (a number) against each peak
...
#nothing, blocks of gray
-   What, in general, is the first argument of the `ggplot()` function?
#which variable you want to graph, in this case it would be hikes!
-   What is the purpose of writing `x = rating`?
#to define the variable of the x axis for graph, also for functions
-   What do you think `aes` stands for?!?
#stands for aesthetic, construct aesthetic mappings (visual part of the graph?)
...
ggplot(hikes, aes(x = rating)) +
  geom_bar()
```
#rating is below the x-axis, hikes is the argument
```{r}
# COMMENT on the change in the code and the corresponding change in the plot
ggplot(hikes, aes(x = rating)) +
  geom_bar() +
...
```
#same as previous, except this time y-axis is defined as "Number of hikes"
```{r}
# COMMENT on the change in the code and the corresponding change in the plot
ggplot(hikes, aes(x = rating)) +
  geom_bar(fill = "blue") +
  labs(x = "Rating", y = "Number of hikes")
```
#the bar color has changed to blue!
```{r}
# COMMENT on the change in the code and the corresponding change in the plot
ggplot(hikes, aes(x = rating)) +
  ...
  labs(x = "Rating", y = "Number of hikes")
```
#change in code: (color="orange", fill="blue")
#change in plot: bars are now orange outlined, filled with blue
```{r}
# COMMENT on the change in the code and the corresponding change in the plot
ggplot(hikes, aes(x = rating)) +
  geom_bar(color = "orange", fill = "blue")  +
  ...
  theme_minimal()
```
#change in code: add theme_minimal
#makes background more simple
...
#adds another idea to the code

-   We added the bars using `geom_bar()`? Why "geom"?
#makes ggplot code a visual object with the data (in this case, it's a bar graph)

-   What does `labs()` stand for?
#modify axis, legend, and plot labels (this allowed us to modify the x and y axis)

-   What's the difference between `color` and `fill`?
#color is the outline of the actual bar outline, fill is the color inside the bar
...
#not evenly spread, moderate is the most common

We must then *translate* this information into the *context* of our analysis, here hikes in the Adirondacks. Summarize below what you learned from the bar chart, in context.
#moderate hikes are the most common


#### Part c

Is there anything you don't like about this barplot? For example: check out the x-axis again.
#order of variables: "difficult, easy, moderate" is sort of a weird order!
...
#too many x variables, visually crowded
...
#The most common elevation of hikes is between 4000 and 5000 feet. The number of hikes is between 1-10.
...
ggplot(hikes, aes(x = elevation)) +
  geom_histogram()
```
#plots number of hikes in a histogram (code is "geom_histogram"), "aes(x=elevation)" defines x-axis as elevation
```{r}
# COMMENT on the change in the code and the corresponding change in the plot
ggplot(hikes, aes(x = elevation)) +
